...
class LoadLabResults:
    ...
    def load_non_numerical_values_and_coerce_inequalities(
        ...
        sql = f"SELECT {cols} FROM [fct].{view} WHERE {npu_where} AND numerisksvar IS NULL AND (left(Svar,1) = '>' OR left(Svar, 1) = '<')"
    ...
    def load_numerical_values(blood_sample_id: str, n: int, view: str) -> pd.DataFrame:
        ...
        sql = f"SELECT {cols} FROM [fct].{view} WHERE {npu_where} AND numerisksvar IS NOT NULL"
        df = sql_load(
            sql,
            database="USR_PS_FORSK",
            chunksize=None,
    ...
    def load_cancelled(blood_sample_id: str, n: int, view: str) -> pd.DataFrame:
        ...
        sql = f"SELECT {cols} FROM [fct].{view} {npu_where} AND Svar == 'Aflyst' AND (left(Svar,1) == '>' OR left(Svar, 1) == '<')"
    ...
    def load_all_values(blood_sample_id: str, n: int, view: str) -> pd.DataFrame:
        ...
        sql = f"SELECT {cols} FROM [fct].{view} WHERE {npu_where}"
